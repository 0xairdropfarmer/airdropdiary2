    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.strategies.update", [$strategy->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label for="name">{{ trans('cruds.strategy.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $strategy->name) }}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="tasks[]" id="tasks" multiple>
                                @foreach($tasks as $id => $task)
                                    <option value="{{ $id }}" {{ (in_array($id, old('tasks', [])) || $strategy->tasks->contains($id)) ? 'selected' : '' }}>{{ $task }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('tasks'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="expire_date" id="expire_date" value="{{ old('expire_date', $strategy->expire_date) }}">
...
<script>
    ...
    url: '{{ route('frontend.strategies.storeMedia') }}',
    maxFilesize: 2, // MB
