        ...
        <div class="col-md-12">
            @can('recurring_activity_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.recurring-activities.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-RecurringActivity">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $recurringActivity->id }}">
                                        ...
                                        <td>
                                            @can('recurring_activity_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.recurring-activities.show', $recurringActivity->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('recurring_activity_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.recurring-activities.edit', $recurringActivity->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('recurring_activity_delete')
                                                <form action="{{ route('frontend.recurring-activities.destroy', $recurringActivity->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('recurring_activity_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.recurring-activities.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-RecurringActivity:not(.ajaxTable)').DataTable({ buttons: dtButtons })
