    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Project::LIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('live') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="live_{{ $key }}" name="live" value="{{ $key }}" {{ old('live', $project->live) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Project::AIRDROPCF_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('airdropcf') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="airdropcf_{{ $key }}" name="airdropcf" value="{{ $key }}" {{ old('airdropcf', $project->airdropcf) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('raisefund') ? 'is-invalid' : '' }}" type="number" name="raisefund" id="raisefund" value="{{ old('raisefund', $project->raisefund) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $project->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="cover">{{ trans('cruds.project.fields.cover') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.cover_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($project) && $project->cover)
      var file = {!! json_encode($project->cover) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
