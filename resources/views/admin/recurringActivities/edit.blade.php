...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.recurringActivity.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.recurring-activities.update", [$recurringActivity->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $recurringActivity->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('task') ? 'is-invalid' : '' }}" name="task_id" id="task_id">
                    @foreach($tasks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('task_id') ? old('task_id') : $recurringActivity->task->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('task'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_interact') ? 'is-invalid' : '' }}" type="number" name="total_interact" id="total_interact" value="{{ old('total_interact', $recurringActivity->total_interact) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_amount') ? 'is-invalid' : '' }}" type="number" name="total_amount" id="total_amount" value="{{ old('total_amount', $recurringActivity->total_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_gas_spend') ? 'is-invalid' : '' }}" type="number" name="total_gas_spend" id="total_gas_spend" value="{{ old('total_gas_spend', $recurringActivity->total_gas_spend) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\RecurringActivity::DONE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('done') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="done_{{ $key }}" name="done" value="{{ $key }}" {{ old('done', $recurringActivity->done) === (string) $key ? 'checked' : '' }}>
