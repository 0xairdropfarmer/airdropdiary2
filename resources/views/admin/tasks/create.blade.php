    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Task::PHASE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('phase') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="phase_{{ $key }}" name="phase" value="{{ $key }}" {{ old('phase', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expire_date') ? 'is-invalid' : '' }}" type="text" name="expire_date" id="expire_date" value="{{ old('expire_date') }}">
