    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('membership_type') ? 'is-invalid' : '' }}" name="membership_type" id="membership_type">
                    ...
                    @foreach(App\Models\User::MEMBERSHIP_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('membership_type', 'annually') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expire_at') ? 'is-invalid' : '' }}" type="text" name="expire_at" id="expire_at" value="{{ old('expire_at') }}">
